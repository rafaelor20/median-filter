...
png_bytep *median_filter_openmp(png_bytep *row_pointers, const int width, const int height, const int window_side) {
    int number_of_channels = 3; // RGB
...
int main(int argc, char **argv) {
    ...
    // Apply the median filter using OpenMP
    int window_side = 5;
    png_bytep *output_image = median_filter_openmp(row_pointers, width, height, window_side);

    // Write the output PNG file
    write_png_file(argv[2], output_image, width, height, color_type, bit_depth);
